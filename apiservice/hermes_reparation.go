...
func GetCommonData(startEpoch, epochCount uint64) (map[uint64][]string, map[string]map[uint64]*HermesDistributionPlan, map[string]map[uint64]*HermesDistributionSource, error) {
	...
	if err != nil {
		return nil, nil, nil, errors.Wrap(err, "failed to get reward distribution plan")
	}
	...
	if err != nil {
		return nil, nil, nil, errors.Wrap(err, "failed to get account rewards")
	}
	return delegateMap, distributePlanMap, accountRewardsMap, nil
}

func GetHermeOrigin(delegateMap map[uint64][]string, distributePlanMap map[string]map[uint64]*HermesDistributionPlan, accountRewardsMap map[string]map[uint64]*HermesDistributionSource) (HermesDistributionReward, error) {
...
func GetHermeFixed(delegateMap map[uint64][]string, distributePlanMap map[string]map[uint64]*HermesDistributionPlan, accountRewardsMap map[string]map[uint64]*HermesDistributionSource) (HermesDistributionReward, error) {

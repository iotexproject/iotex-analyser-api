...
// grpcurl -plaintext -d '{"startEpoch": 22416, "epochCount": 1, "rewardAddress": "io12mgttmfa2ffn9uqvn0yn37f4nz43d248l2ga85"}' 127.0.0.1:8888 api.AccountService.HermesBucket
func (s *HermesService) HermesBucket(ctx context.Context, req *api.HermesRequest) (*api.HermesBucketResponse, error) {
	resp := &api.HermesBucketResponse{}
	startEpoch := req.GetStartEpoch()
	epochCount := req.GetEpochCount()
	rewardAddress := req.GetRewardAddress()
	endEpoch := startEpoch + epochCount - 1
	...
	voterVotesMap, err := rewards.BucketIDWeightedVotesBySearchPairs(delegateMap)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get voter votes")
	}
	hermesBucketDistributions := make([]*DelegateHermesBucketDistribution, 0, len(accountRewardsMap))
	for delegate, rewardsMap := range accountRewardsMap {
		planMap := distributePlanMap[delegate]
		epochVoterMap := voterVotesMap[delegate]

		voterAddrToReward := make(map[string]map[uint64]*big.Int)
		balanceAfterDistribution := big.NewInt(0)
		voterCountMap := make(map[string]bool)
		feeWaiver := true
		var stakingAddress string
		...
		for epoch, rewards := range rewardsMap {
			...
			for voterAddr, bucketMap := range voterMap {
				if _, ok := voterAddrToReward[voterAddr]; !ok {
					voterAddrToReward[voterAddr] = make(map[uint64]*big.Int)
				}
				bucketToReward := voterAddrToReward[voterAddr]

				for bucketID, weightedVotes := range bucketMap {
					amount := new(big.Int).Set(distrReward)
					amount = amount.Mul(amount, weightedVotes).Div(amount, distributePlan.TotalWeightedVotes)
					if _, ok := bucketToReward[bucketID]; !ok {
						bucketToReward[bucketID] = big.NewInt(0)
					}
					bucketToReward[bucketID].Add(bucketToReward[bucketID], amount)
					balanceAfterDistribution.Sub(balanceAfterDistribution, amount)
				}
				voterCountMap[voterAddr] = true
			}
		}
		bucketRewardDistribution, err := convertVoterBucketDistributionMapToList(voterAddrToReward)
		if err != nil {
			return nil, errors.Wrap(err, "failed to convert voter distribution map to list")
		}
		hermesBucketDistributions = append(hermesBucketDistributions, &DelegateHermesBucketDistribution{
			DelegateName:        delegate,
	...
	hermesBucketDistribution := make([]*api.HermesBucketDistribution, 0, len(hermesBucketDistributions))
	for _, ret := range hermesBucketDistributions {
		rds := make([]*api.BucketRewardDistribution, 0)
		for _, distribution := range ret.Distributions {
			v := &api.BucketRewardDistribution{
				VoterEthAddress:   distribution.VoterEthAddress,
				VoterIotexAddress: distribution.VoterIotexAddress,
				BucketID: distribution.BucketID,
				Amount:            distribution.Amount,
			}
			rds = append(rds, v)
		}
		...
		hermesBucketDistribution = append(hermesBucketDistribution, &api.HermesBucketDistribution{
			DelegateName:        ret.DelegateName,
			BucketRewardDistribution:  rds,
			StakingIotexAddress: ret.StakingIotexAddress,
			VoterCount:          ret.VoterCount,
			WaiveServiceFee:     ret.WaiveServiceFee,
			Refund:              ret.Refund,
	...
	sort.Slice(hermesBucketDistribution, func(i, j int) bool { return hermesBucketDistribution[i].DelegateName < hermesBucketDistribution[j].DelegateName })
	resp.HermesBucketDistribution = hermesBucketDistribution
	return resp, nil
}

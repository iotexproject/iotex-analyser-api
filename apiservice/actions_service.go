...
/*
select t.*,(select action_type from block_action_partition where action_hash=t.action_hash),(select timestamp from block where block_height=t.block_height) from ((select a.block_height,a.action_hash,a.amount,a.sender,a.recipient, 'xrc20' as rtype from token_erc20 a where a.sender='io15qr5fzpxsnp7garl4m7k355rafzqn8grrm0grz' or a.recipient='io15qr5fzpxsnp7garl4m7k355rafzqn8grrm0grz') union
        (SELECT a.block_height,a.action_hash,a.amount,a.sender,a.recipient, 'native' as rtype FROM block_action_partition a where a.sender='io15qr5fzpxsnp7garl4m7k355rafzqn8grrm0grz' or a.recipient='io15qr5fzpxsnp7garl4m7k355rafzqn8grrm0grz') union
...
select t.* from ((select a.block_height,a.action_hash,a.amount,a.sender,a.recipient, 'xrc20' as rtype from token_erc20 a where a.sender='io1hp6y4eqr90j7tmul4w2wa8pm7wx462hq0mg4tw' or a.recipient='io1hp6y4eqr90j7tmul4w2wa8pm7wx462hq0mg4tw' order by block_height desc) union all
        (SELECT a.block_height,a.action_hash,a.amount,a.sender,a.recipient, 'native' as rtype FROM block_action_partition a where a.sender='io1hp6y4eqr90j7tmul4w2wa8pm7wx462hq0mg4tw' or a.recipient='io1hp6y4eqr90j7tmul4w2wa8pm7wx462hq0mg4tw' order by block_height desc) union all
...
func (s *ActionsService) GetAllActionsByAddress(ctx context.Context, req *api.ActionsRequest) (*api.AllActionsByAddressResponse, error) {
	...
	resp.Count = uint64(count)
	resQuery := `select t.*,(select action_type from block_action_partition where action_hash=t.action_hash),(select timestamp from block where block_height=t.block_height) from (` + query + `)t order by block_height ` + sort + ` limit ? offset ?`

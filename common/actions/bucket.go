...
func GetBucketActionInfoByBuckets(bucketIDs []uint64, skip, first uint64) ([]*BucketActionInfo, error) {
	var bucketActionInfos []*BucketActionInfo
	db := db.DB()
	query := "select distinct action_hash act_hash,t2.action_type act_type,t2.sender,t2.recipient,t2.amount,t2.gas_price*t2.gas_consumed as gas_fee,t2.block_height blk_height,t1.block_hash blk_hash,t1.timestamp time_stamp from  block_action t2 left join block t1 on t1.block_height=t2.block_height  where t2.action_hash in (select distinct t1.act_hash from staking_actions t1 where t1.bucket_id in (?)) order by t1.timestamp desc limit ? offset ?"
	if err := db.Raw(query, bucketIDs, first, skip).Scan(&bucketActionInfos).Error; err != nil {
		return nil, err
	}
	return bucketActionInfos, nil
}
